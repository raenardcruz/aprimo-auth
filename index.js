    ...
    connect() {
        return new Promise((resolve, reject) => {
            ...
            if (!params.has("code")) {
                ...
                window.location.href = `https://${this.subdomain}.aprimo.com/login/connect/authorize?response_type=code&state=12345&client_id=${this.clientid}&redirect_uri=${encodeURIComponent(this.redirecturi)}&scope=api+offline_access&code_challenge_method=S256&code_challenge=${codeChallenge}`;
            }
        })
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            // Step 2: Call Aprimo Authentication URL
                ...
                .post(`https://${this.subdomain}.aprimo.com/login/connect/token`,
                    `grant_type=authorization_code&code=${params.get("code")}&redirect_uri=${encodeURIComponent(this.redirecturi)}&client_id=${this.clientid}&code_verifier=${codeVerifier}&client_secret=${this.secret}`,
                    {
